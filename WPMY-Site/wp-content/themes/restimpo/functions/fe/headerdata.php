...
global $restimpo_options_db;
    ...
    if ($restimpo_options_db['restimpo_css'] == 'Lime' ){
			wp_enqueue_style('style-lime', get_template_directory_uri().'/css/lime.css');
		}
    
		if ($restimpo_options_db['restimpo_css'] == 'Pink' ){
			wp_enqueue_style('style-pink', get_template_directory_uri().'/css/pink.css');
		}
}
...
function restimpo_get_homepage_header_image() { 
    $homepage_header_image = get_header_image();
    $homepage_header_image_text_height = (get_custom_header()->height)*0.5; 
		if ($homepage_header_image != '') { ?>
		<?php _e('#wrapper #wrapper-header .header-image { background-image: url(', 'restimpo'); ?><?php esc_url(header_image()); ?><?php _e('); height: ', 'restimpo'); ?><?php echo get_custom_header()->height; ?><?php _e('px; } .header-image .header-image-text .header-image-info { max-height: ', 'restimpo'); ?><?php echo $homepage_header_image_text_height ?><?php _e('px; }', 'restimpo'); ?>
<?php } else { ?> 
<?php _e('#wrapper #wrapper-header .header-image { display: none; }', 'restimpo'); ?>
<?php } 
}

// Homepage Header Image Size
function restimpo_get_header_image_size() {
global $restimpo_options_db;
    $header_image_size = $restimpo_options_db['restimpo_header_image_size']; 
		if ($header_image_size != '' && $header_image_size != 'Cover') { ?>
		<?php _e('#wrapper #wrapper-header .header-image{ background-size: auto; }', 'restimpo'); ?>
<?php } 
}
...
global $restimpo_options_db;
    $headline_box_google_fonts = $restimpo_options_db['restimpo_headline_box_google_fonts']; 
		...
		<?php _e('#wrapper .container #main-content .entry-headline { font-family: "', 'restimpo'); ?><?php echo $headline_box_google_fonts ?><?php _e('", Arial, Helvetica, sans-serif; }', 'restimpo'); ?>
<?php } 
}
...
<?php restimpo_get_homepage_header_image(); ?>
<?php restimpo_get_header_image_size(); ?>
